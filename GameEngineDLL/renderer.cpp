...
void Renderer::Begin(void * Resource, bool NewSurface)
{
	...
	if (NewSurface)
	{
		...
		if (FAILED(hr))
		{
			MessageBox(NULL, "QueryInterface", "ERROR", MB_ICONHAND | MB_OK);
		...
		hr = m_Device->CreateRenderTargetView(pOutputResource, &rtDesc, &m_RenderTargetView);
		if (FAILED(hr))
		{
			MessageBox(NULL, "QueryInterface", "ERROR", MB_ICONHAND | MB_OK);
			//return hr;
		...
		m_ScreenWidth = outputResourceDesc.Width;
		m_ScreenHeight = outputResourceDesc.Height;
...
void Renderer::SetWorldViewProjection2D()
{
	...
	D3DXMatrixOrthoOffCenterLH(&projection, 0.0f, m_ScreenWidth, m_ScreenHeight, 0.0f, 0.0f, 1.0f);
